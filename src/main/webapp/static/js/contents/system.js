...
window.adjustSystemContainerWidthBasedOnScroll = function () {
    // 스크롤바 너비 조절 관련 요소들
    var $contentWrapper = $('.content-wrapper');
    var $authSystemContainer = $('.auth-system-container');

    if ($authSystemContainer.length === 0 || $contentWrapper.length === 0) {
        return;
    }
    if ($contentWrapper[0].scrollHeight > $contentWrapper[0].clientHeight) {
        $authSystemContainer.addClass('scrolling');
    } else {
        $authSystemContainer.removeClass('scrolling');
    }
};
