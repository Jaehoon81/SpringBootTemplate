...
window.initDataStatistics = function () {  // 데이터 통계와 페이징 리스트 로드 함수를 전역 스코프에 노출
    ...
    function loadParticipantList(page) {
        currentPage = page;
        $('#participant-list-body').html('<tr><td colspan="8" class="loading-contents">참가자 목록을 불러오는 중...</td></tr>');
        $('#no-data-message').hide();

        $.ajax({
            url: `/api/participants/paginated-list?page=${currentPage}&size=${listPerPage}`,
            type: 'GET',
            headers: {
            ...
            success: function (response) {
                ...
                // 페이지네이션 컨트롤 그리기
                renderPaginationControls(totalPages, currentPage, listPerPage, loadParticipantList);
    ...
    $('#goToNoticeLink').click(function (e) {
        e.preventDefault();  // 기본 링크 동작 방지
        const targetUrl = '/contents/notice';  // 공지사항 내용을 가져올 URL
        loadDataContent(targetUrl);
    });

    // 데이터(Statistics, Notice) 콘텐츠 로드 함수 -------------------------------------------------------------------------
    function loadDataContent(url, title = '데이터 통계') {
