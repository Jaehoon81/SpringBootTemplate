...
window.initDataStatistics = function () {  // 데이터 통계와 페이징 리스트 로드 함수를 전역 스코프에 노출
    ...
    function loadParticipantList(page, gradeFilter, startDate, endDate) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                renderPaginationControls(totalPages, currentPage, listPerPage, function (targetPage) {
                    loadParticipantList(targetPage, currentGradeFilter, currentStartDate, currentEndDate);
                });
                // 콘텐츠 로드 후 스크롤 상태를 확인
    ...
    $(document).off('change', 'input[name="gradeFilter"]')
        ...
        loadParticipantList(1, selectedGrade, currentStartDate, currentEndDate);
    });
    ...
    $dateRangePicker.daterangepicker({
        ...
        autoUpdateInput: false,       // 입력 필드의 자동 업데이트 끔 (직접 포맷팅)
        ...
        locale: {  // 직접 한국어로 텍스트 정의
            format: 'YYYY/MM/DD',  // 입력 필드에 표시될 날짜 형식
        ...
        setTimeout(function () {
            if (!isCalendarApplied) {  // 캘린더 외부를 클릭했을 경우로 간주
                // 입력 필드를 마지막으로 선택했던 값으로 되돌림 (혹은 기본값으로 원복)
    ...
    $dateRangePicker.on('apply.daterangepicker', function (ev, picker) {
        ...
        if (newStartDate !== currentStartDate || newEndDate !== currentEndDate) {
            ...
            loadParticipantList(1, currentGradeFilter, newStartDate, newEndDate);
        }
    ...
    window.changeParticipantGrade = function (participantId, participantName, newGrade) {
        if (!confirm(`참가자, ${participantName}님의 등급을 ${newGrade}(으)로 변경하시겠습니까?`)) {
            ...
            loadParticipantList(currentPage, currentGradeFilter, currentStartDate, currentEndDate);
            return;
        }
        $.ajax({
    ...
    window.loadDataContent = function (targetUrl, pushState = true) {
        var $dataContentArea = $('#dataContentArea');
    ...
    if (savedBackPressed === 'true') {
        ...
        if (savedGradeFilter) {
            ...
            $('input:radio[name="gradeFilter"]:input[value=""]').prop('checked', true);
        }
