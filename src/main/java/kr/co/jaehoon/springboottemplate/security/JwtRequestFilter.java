...
//                throw new BadCredentialsException("토큰이 무효화되었습니다. 다시 로그인해주세요.");
            ...
            try {
                ...
                unauthorizedMsg = "Malformed JWT token";
                log.warn("{}: {}", unauthorizedMsg, e.getMessage());
...
//                throw new BadCredentialsException("사용자 정보를 불러올 수 없습니다.");
            ...
            if (userDetails != null) {
                ...
                if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {  // 모바일 토큰의 경우 JTI 검증
                    if (user != null && jti != null
                            && user.getActiveSessionJti() != null && !user.getActiveSessionJti().equals(jti)) {
                        ...
                        create401JsonResponse(request, response, "다른 기기에서 로그인했거나 세션이 만료되었습니다.\n다시 로그인해주세요.", shouldSendJson);
                        return;  // 필터 체인 진행을 중단
                    } else if (user != null && jti != null
                            && user.getActiveSessionJti() == null) {
